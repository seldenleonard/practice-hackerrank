...
# INSTRUCTIONS
# A person wants to determine the most expensive computer keyboard and USB drive that can be purchased with a given budget. Given price lists for keyboards and USB drives and a budget, find the cost to buy them. If it is keyboardsot possible to buy both items, return -1.
...
# Input Format
  # The first line contains three space-separated integers b, keyboards, and m, the budget, the keyboardsumber of keyboard models and the keyboardsumber of USB drive models.
  # The second line contains keyboards space-separated integers keyboard[i], the prices of each keyboard model.
  # The third line contains m space-separated integers drives, the prices of the USB drives.

# STEPS
  # Write a method that accepts three inputs, array keyboards (prices of keyboard models), array drives (prices of USB drive models), and integer b (budget).
...
def getMoneySpent(keyboards, drives, b)
  ...
  if max_spending > b
    ...
    while max_spending < b || index_k < keyboards.length # might consider making this less than OR EQUAL TO
      index_k += 1
      if keyboards[index_k].to_i + drives[index_d].to_i < b
        max_spending = keyboards[index_k] + drives[index_d]
        if max_spending == b
          b
        end
